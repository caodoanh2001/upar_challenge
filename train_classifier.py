...
def main(cfg, args):
    ...
    timestamp = time_str()
    stdout_file = os.path.join(log_dir, f'stdout_finetune_classifier.txt')
    save_model_path = os.path.join(model_dir, f'ckpt_max_finetune_classifier.pth')
...
def trainer(cfg, args, epoch, model, model_ema, train_loader, valid_loader, criterion, optimizer, lr_scheduler,
    ...
    ckpt = torch.load("/home/compu/doanhbc/upar_challenge/SOLIDER-PersonAttributeRecognition/exp_result/PA100k/swin_b.sm08/img_model/ckpt_max_evavit_swinT_best_avg_acc_7134.pth")
